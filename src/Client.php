...
class Client
{
    private $providerFederalTaxNumber = '';
    private $providerTaxpayerRegister = '';
    private $passphrase;
    private $pkcs12;
    private $certDir;
    private $privateKey;
    ...
    {
        $this->certDir = $configuration['temporary']['dir'] ?? '/tmp';
        $this->pkcs12 = $configuration['certificate']['pkcs12'];
        $this->passphrase = $configuration['passphrase'];
        $this->providerFederalTaxNumber = $configuration['cnpj'];
        $this->providerTaxpayerRegister = $configuration['ccm'];
    ...
    private function loadCert()
    {
        ...
        if (!file_exists($this->privateKey) || md5_file($this->privateKey) != md5($x509CertData['pkey'])) {
        ...
        if (!file_exists($this->publicKey) || md5_file($this->publicKey) != md5($x509CertData['cert'])) {
        ...
        if (!file_exists($this->key) || md5_file($this->key) != md5($x509CertData['cert'] . $x509CertData['pkey'])) {
    ...
    private function createXML($operation)
    {
        ...
        $data = '<?xml version="1.0" encoding="UTF-8"?><pito:Pedido' . $operation . ' xmlns:xsd="' . $this->urlXsd . '" xmlns:pito="' . $this->urlNfe . '" xmlns:xsi="' . $this->urlXsi . '"></pito:Pedido' . $operation . '>';
    ...
    private function signRPS(NFeRPS $rps, &$rpsNode)
    {
        ...
        $content = sprintf('%08s', $rps->taxPayerRegisterProvider) .
            ...
            ($type == 'F' ? '1' : ($type == 'C' ? '2' : '3')) .
            sprintf('%014s', $rps->contractorRPS->federalTaxNumber ?: 0);
    ...
    private function insertRPS(NFeRPS $rps, &$xmlDoc)
    {
        ...
        if ($rps->contractorRPS->type == 'F') {
            $cpf = $xmlDoc->createElement('CPFCNPJTomador'); // 0-1
            $cpf->appendChild($xmlDoc->createElement('CPF', sprintf('%011s', $rps->contractorRPS->federalTaxNumber)));
            $rpsNode->appendChild($cpf);
            $rpsNode->appendChild($xmlDoc->createElement('RazaoSocialTomador', htmlspecialchars($rps->contractorRPS->name))); // 0-1
        } elseif ($rps->contractorRPS->type == 'C') {
...
//    $address->appendChild( $xmlDoc->createElement( 'TipoLogradouro', $rps->contractorRPS->addressType ) );
        $address->appendChild($xmlDoc->createElement('Logradouro', htmlspecialchars(substr($rps->contractorRPS->address, 0, static::ADDRESS_MAX_LENGHT))));
        $address->appendChild($xmlDoc->createElement('NumeroEndereco', substr($rps->contractorRPS->addressNumber, 0, static::NUMBER_MAX_LENGHT)));
        $address->appendChild($xmlDoc->createElement('ComplementoEndereco', substr($rps->contractorRPS->complement, 0, static::COMPLEMENT_MAX_LENGHT)));
        $address->appendChild($xmlDoc->createElement('Bairro', htmlspecialchars($rps->contractorRPS->district)));
     ...
     * @param array $totalValue ( 'services' => total value of RPSs, 'deductions' => total deductions on values of RPSs )
    ...
    public function sendRPSBatch($rangeDate, $totalValue, $rps)
    {
        ...
        $header->appendChild($xmlDoc->createElement('ValorTotalServicos', $totalValue['services']));
...
//    echo $xmlDoc->saveXML();
    ...
    public function cancelNFe($nfeNumbers)
    {
        ...
        foreach ($nfeNumbers as $nfeNumber) {
            ...
            $signatureValue = shell_exec("echo -n $content | openssl dgst -sha1 -sign {$this->privateKey}");
    ...
    public function queryNFe($nfeNumber, $rpsNumber = null, $rpsSerie = null)
    {
    ...
    public function textFile($rangeDate, $totalValue, $rps)
    {
        ...
        $footer = "9" .
            ...
            sprintf("%015s", str_replace('.', '', sprintf('%.2f', $totalValue['services']))) .
